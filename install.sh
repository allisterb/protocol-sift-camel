...
for s in preserve_chatlog.py generate_pdf_report.py; do
    src="$REPO_DIR/analysis-scripts/$s"
    if [[ -f "$src" ]]; then
        cp "$src" "$CLAUDE_DIR/analysis-scripts/$s"
        ok "  $s → $CLAUDE_DIR/analysis-scripts/"
    else
        warn "  analysis-scripts/$s not found, skipping."
    fi
done
echo
