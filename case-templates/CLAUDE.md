...
- All forensic tool execution and the chain-of-custody audit log are handled by Camel
  (`audit-<caseId>.clef`). Do not run forensic CLIs directly — route everything through Camel even
  though raw shell access is permitted (it is left enabled only to match the baseline's posture).
- Timestamps: always report in UTC.
- Reports: write Markdown to `./reports/`; for a PDF, use `./analysis/generate_pdf_report.py`.
